 ...
 */
public class CRC16CheckSum implements Converter {
	@Override
	public ConversionResult convert(String input) {
		...
		return new ConversionResult().withResult(checksum(input.getBytes(Charset.defaultCharset())));
	}

	private String checksum(byte[] bytes) {
		int crc = 0x0000;
		for (byte b : bytes) {
			crc = (crc >>> 8) ^ lookup[(crc ^ b) & 0xff];
		}
